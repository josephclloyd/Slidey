...
- **CoreML model conversion: always set `compute_precision=ct.precision.FLOAT32` explicitly on `ct.convert()`.** coremltools defaults `mlprogram` conversion to FLOAT16. SwinIR's conversion took this default and it overflowed (see `slidey/Resources/convert_swinir.py`'s "Precision" section and PR #251) because the model scales activations 255x internally; NAFNet's conversion (`slidey/Resources/convert_nafnet.py`) sets FLOAT32 from the start specifically to avoid re-learning this lesson. Do this for any future model conversion regardless of whether the specific architecture has an obvious overflow risk — the failure mode (silent 100% NaN output, not a conversion error) is expensive to diagnose after the fact.
- **A custom `torch.autograd.Function` (e.g. a hand-written backward pass for training-time memory efficiency) traces as an opaque, unconvertible `pythonop` node.** Hit converting NAFNet: `LayerNorm2d` wraps `LayerNormFunction.apply()`, a custom autograd Function used only for backward-pass efficiency during training. `torch.jit.trace` records the call itself rather than inlining its constituent tensor ops, and coremltools' torch frontend has no generic handler for it (`RuntimeError: PyTorch convert function for op 'pythonop' not implemented`). Fix: monkey-patch the module's `.forward` to the equivalent plain-tensor-op implementation before tracing (verified bit-exact against the original in `convert_nafnet.py`) — inference doesn't need a custom backward, so there's no reason to preserve the autograd.Function wrapper for conversion. If a future model conversion hits `pythonop`, look for `torch.autograd.Function` subclasses in the architecture first.
