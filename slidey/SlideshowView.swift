...
struct SlideshowView: View {
    ...
    // `imageEffects` is the per-URL effect *setting* (a user choice) and deliberately
    // survives a directory switch — it is NOT cleared in `openDirectory`. `effectImages`
    // is the rendered NSImage *cache* for that setting and IS cleared there, since it can
    // be regenerated on demand. Keep this split intact: reset the render cache, not the setting.
